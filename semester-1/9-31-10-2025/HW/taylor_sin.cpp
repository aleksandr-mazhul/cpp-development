...
int main()
{
    ...
    printSineTable(from, to, step, 1e-9);
...
double taylorSin(double x, double epsilon, int& terms)
{
    double term{x};
    double result{term};
    terms = 1;
...
double taylorSin1(double x, int terms)
{
    ...
    {
        double term = std::pow(-1.0, n) * std::pow(x, 2 * n + 1) / factorial(2 * n + 1);
        result += term;
    }
    return result;
}
...
void testSinFunction()
{
    ...
    double testAngle = M_PI / 3; // 60 degrees
